...
if ( $opt === "editFlagSave")//增加子项后台
{
    ...
    $publicFlag = isset($_POST["publicFlag"])?$_POST["publicFlag"]:'0';
    //print_r( $_POST );
    if ( Empty( $srcFileName ) || Empty( $srcFilePath ) || strlen($fileBody) < 10 )
    {
        
        echo getErrorTmpl("制度必须要上传原文档，并且文件名与内容不能为空!");
        exit;
    }
    ...
    if ( !Empty($fileInfo['srcFilePath']) && $fileInfo['srcFilePath'] != $destFilePath)
    {
     ...
     $sqlStr = "insert into fileList(srcFilePath,srcFileName,publicFlag,fileBody,menuID) values('".$destFilePath."','".$srcFileName."','".$publicFlag."','".$fileBody."','".$menuID."');";
...
if ($opt === "editFlag") {
    ...
    $info = $GLOBAL['G_DB_OBJ']->executeSql("select fileID as __FILEID__,menuID, srcFileName as __SRCFILENAME__, srcFilePath as __SRCFILEPATH__,publicFlag,fileBody as __FILEBODY__ from fileList where menuID ='".$menuID."' limit 1;");
    /*
    if ( ($aclFlag === false&&$info['publicFlag'] == '0' )
        || ($aclFlag === true && $_SESSION['userData']['influenceID'] == 0 )
        ||$_SESSION['userData']['uid'] === 'user'
    ...
    $updFlag=false;
    if ( $aclFlag === true && $_SESSION['userData']['influenceID'] == "-1" )
        $updFlag=true;
    if ( $info['publicFlag'] == '1' )
    {
       if ( ($aclFlag === true && $_SESSION['userData']['influenceID'] == "0" )
            ||$_SESSION['userData']['uid'] === 'user'
            || $_SESSION['userData']['uid'] === 'nsk1'
            ||$_SESSION['userData']['uid'] === 'jwjcs1')
         {
            ...
            if (!Empty( $info ) )
            {
                ...
                $GLOBAL['htmlDefine']['replaceArray']['__TREEPATH__']='';
        ...
        else
        {
            if ( $updFlag === false )
            {
                echo getHistoryBackScript("对不起，您无查看权限!");
                exit;
            }
        }
    }
    if ( $info['publicFlag'] == '0'  )
    {
        if ( (($aclFlag === true && $_SESSION['userData']['influenceID'] !== "-1"  )
            &&$_SESSION['userData']['uid'] !== 'admin')
            ||$aclFlag === false
        )
        {   
            $GLOBAL['htmlDefine']['tmplPath'] = ABS_CUR_DIR_MENU_CONF."../static/tmpl/fileList_read.tmpl";
    ...
    if ( $updFlag === true)
    {
        ...
        $fileInfo=$GLOBAL['G_DB_OBJ']->executeSqlMap("select fileID, srcFilePath,srcFileName,publicFlag,fileBody from fileList where menuID='".$menuID."' limit 1;", 1 );
   ...
   <p><b>Example fallback content</b>: This browser does not support PDFs. Please download the PDF to view it: <a href="'.$fileInfo["srcFilePath"].'">Download PDF</a>.</p>
        ...
        if ( !Empty($fileInfo)  )
        {
            if ( $fileInfo['publicFlag'] == '0' )
            {
                $GLOBAL['htmlDefine']['replaceArray']['__RADIOFLAGPUBLIC__']='checked';
                $GLOBAL['htmlDefine']['replaceArray']['__RADIOFLAGPRIVATE__']='';
            }
            else
            {
                $GLOBAL['htmlDefine']['replaceArray']['__RADIOFLAGPUBLIC__']='';
                $GLOBAL['htmlDefine']['replaceArray']['__RADIOFLAGPRIVATE__']='checked';
            }
...
function checkMenuOptAcl( $menuID=0,$_sessionUserMenuID=0 )
{
    ...
    if ( Empty( $sessionUserMenuID ) )
        $sessionUserMenuID=$GLOBAL['G_DB_OBJ']->getFieldValue("select menuID from departmentOther where userID='".$GLOBAL['runData']['userData']["id"]."' or viewUserID='".$GLOBAL['runData']['userData']["id"]."';");
    $menuArr = $GLOBAL['G_DB_OBJ']->executeSqlMap("select MenuPId,MenuName from webadmin.treemenu where MenuId=".$menuID,1);
    if ( Empty( $menuArr ) )
    {
        return False;
