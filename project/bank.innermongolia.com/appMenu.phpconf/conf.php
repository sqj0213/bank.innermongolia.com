...
if ($opt === "listFlag") {
    /*
    $departmentList = $GLOBAL['G_DB_OBJ']->executeSqlMap('select id,name,note,userID from department');
    ...
    for ( $i=0; $i < count( $departmentList ); $i++ )
    {
        ...
        $htmlStr .= getMenuLevel2("填加目录", "/fileEditOther.php?opt=addFlag&departmentID=".$departmentList[$i]['id']."&fileType=workFlow");
    ...
    if ( $_SESSION['userData']['uid'] === 'user1'  )
        $subSqlStr = " and menuID<>214 and MenuPId<>214 and menuID<>200 AND MenuPId<>200 and menuID<>196 AND MenuPId<>196 and menuID<>204 AND MenuPId<>204 and menuID<>212 AND MenuPId<>212 and bankID='0'  and MenuName<>'退出系统'";
    else
        $subSqlStr = " and bankID='0' and MenuName<>'退出系统'";
    ...
    $tree_class = new treeDBClass(0, $subSqlStr, -1 );
    $menuStr=$tree_class->buildTreeHtml();
    $htmlStr = getMenuLevel0( $bankName, $menuStr,"display:block" );
    ...
    while ( current( $bankList ) !== false )
    {
        ...
        $bankID = $v1["bankID"];
        $subSqlStr = "";
        $firstSql = "select * from webadmin.treemenu where bankID='".$bankID."' and MenuPId='0' limit 30";

        $tree_class = new treeDBClass(0, $subSqlStr, -1, $firstSql );
        $menuStr=$tree_class->buildTreeHtml();
        $tmpStr .= getMenuLevel0( $bankName, $menuStr,"display:none" );
        next( $bankList );
    }
    $htmlStr .= $tmpStr;
    
     
     $GLOBAL['htmlDefine']['replaceArray']['__MENUBODY__'] = $htmlStr;
    
    $GLOBAL['htmlDefine']['replaceArray']['__RIGHTMENUOPT__']=RIGHTMENUSTR.RIGHTMENUOPTSTR;
    $MenuIDList = $GLOBAL['G_DB_OBJ']->executeSqlMap("select MenuId from webadmin.treemenu where MenuPid=0 and( MenuId=615 or MenuName='退出系统');");
    $MenuIDListStr="";
    ...
    $tmpValue=$GLOBAL['G_DB_OBJ']->executeSqlMap("select MenuId from webadmin.treemenu where MenuPid=0");
    $rootIDListStr="";
    foreach ($tmpValue as  $value)
        $rootIDListStr .= $value["MenuId"].",";
    $GLOBAL['htmlDefine']['replaceArray']['__ROOTIDLIST__']=substr($rootIDListStr,0,-1);
...
if ( $opt === "addSubFlagSave")//增加子项后台
{
    ...
    if ( checkMenuOptAcl($menuID) === False )
    {
        echo getAlertFormJSCode("对不起，您无操作权限!");
        exit;
    }
...
if ($opt === "addSubFlag") {
    ...
    $GLOBAL['htmlDefine']['replaceArray']['__MENUPATH__']=getTreePath1($menuID,"");
...
function checkMenuOptAcl( $menuID=0,$_sessionUserMenuID=0 )
{
    ...
    if ( $sessionUserMenuID === 0 )
        $sessionUserMenuID=$GLOBAL['G_DB_OBJ']->getFieldValue("select menuID from department where userID='".$GLOBAL['runData']['userData']["id"]."';");
    if (Empty( $sessionUserMenuID ) )
    {
        $sessionUserMenuID=$GLOBAL['G_DB_OBJ']->getFieldValue("select menuID from departmentOther where userID='".$GLOBAL['runData']['userData']["id"]."';");
    }
...
function getTreePath1( $menuID=0,$menuPath )
{
    global $GLOBAL;
    $retVal = "";
    $menuArr = $GLOBAL['G_DB_OBJ']->executeSqlMap("select MenuPId,MenuName from webadmin.treemenu where MenuId=".$menuID,1);
    if ( empty( $menuArr ) )
        ...
        if ( $menuArr['MenuPId'] !== 0 )
        {
            ...
            return getTreePath1( $menuArr['MenuPId'],$menuPath );
