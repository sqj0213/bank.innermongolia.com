...
if ($opt === "listFlag") {
    ...
    for ( $i=0; $i < count( $departmentList ); $i++ )
    {
        ...
        $workFlowList = $GLOBAL['G_DB_OBJ']->executeSqlMap("select fileID,name,srcFileName from fileListOther where departmentID='".$departmentList[$i]['id']."' and fileType='question'");
        if ( checkAcl($departmentList[$i]['id']) )
            $htmlStr .= getMenuLevel2("填加项目", "/question.php?opt=addFlag&departmentID=".$departmentList[$i]['id']);
            
        for ( $j = 0; $j < count( $workFlowList ); $j++ )
        {
            $htmlStr .= getMenuLevel2( $workFlowList[$j]['name'],"/question.php?opt=readFlag&departmentID=".$departmentList[$i]['id']."&fileID=".$workFlowList[$j]['fileID']);
        }
        $htmlStr .= '</ul>';
        $htmlStr .= "</dd>";
    }
