...
if ( $opt === "addFlagSave")//增加子项后台
{
    ...
    $sqlStr = "insert into fileListOther(name,srcFilePath,srcFileName,fileBody,departmentID,fileType) values('".$flowChartName."','".$destFilePath."','".$srcFileName."','".$fileBody."','".$departmentID."','question');";

    if ($GLOBAL['G_DB_OBJ']->executeSql($sqlStr) === 1 )
    {
        echo getAlertFormLocationJSCode('/question.php?opt=addFlag&departmentID='.$departmentID,"填加成功","","parent.parent.leftFrame.location.reload(true);");
    }
    else 
    {
...
if ( $opt==="delFlag" )
{
    ...
    if ( $GLOBAL['G_DB_OBJ']->executeSql($sqlStr) !== 1 )
    {
        echo getErrorTmpl('/question.php?opt=editFlag&fileID='.$fileID.'&departmentID='.$departmentID,"保存失败，请联络管理员!");
        exit;
    }
    else 
    {
        echo getAlertFormLocationJSCode('/question.php?opt=addFlag&departmentID='.$departmentID,"删除成功!","","parent.parent.leftFrame.location.reload(true);");
        exit;
    }
}
if ( $opt === "editFlagSave")//增加子项后台
{
    ...
    if ($GLOBAL['G_DB_OBJ']->executeSql($sqlStr) === 1 )
    {
        $fileID=$GLOBAL['G_DB_OBJ']->getFieldValue("select fileID from fileListOther where name='".$flowChartName."' and srcFilePath='".$destFilePath."' and srcFileName='".$srcFileName."' and fileType='question'");
        echo getAlertFormLocationJSCode('/question.php?opt=readFlag&departmentID='.$departmentID.'&fileID='.$fileID,"","","parent.parent.leftFrame.location.reload(true);");
        exit;
    }
    else
    {
