...
if ($opt === "listFlag") {
    ...
    $sql = "insert into user( uid, passwd,userName,influenceID) values('".$uid."','".$pwd."','".$userName."','-1');insert into treemenu(MenuName,MenuPId,MenuEndFlag,SortValue) values('".$name."','0','0','".$sortValue."');";
    ...
    {
        $sql = "insert into department(menuID,userID,note,sortValue) values('".$menuID."','".$userID."','".$note."','".$sortValue."')";
    ...
    $id = $_REQUEST["id"];
    $info = $GLOBAL['G_DB_OBJ']->executeSql("select id,sortValue, (select MenuName from treemenu where MenuId=department.menuID limit 1 ) as name,(select uid from user where id=department.userID limit 1) as uid,(select passwd from user where id=department.userID limit 1) as passwd,note from department where id = $id");
    ...
    if ( !Empty( $userID) )
    {
        ...
        {
            echo getWebLocationScript( '/departmentManage.php?&opt=listFlag', '科室修改成功,帐号修改失败,有同名的帐号存在!' );
            exit;
        }
        else 
        {
            ...
            if ( $GLOBAL['G_DB_OBJ']->executeSql( $sql ) === 1 )
            {
                echo getWebLocationScript( '/departmentManage.php?&opt=listFlag', '科室修改成功,帐号修改成功!' );
                exit;
            }
            else 
            {
                echo getWebLocationScript( '/departmentManage.php?&opt=listFlag', '科室修改成功,帐号修改失败1!' );
                exit;
            }
        }
    ...
    if ( !Empty( $userID ) )
    {
        $sql="delete from user where id='".$userID."';delete from department where id='".$id."';delete from treemenu where MenuId='".$menuID."'";
        $GLOBAL['G_DB_OBJ']->executeMutiSqlTrans( $sql );
