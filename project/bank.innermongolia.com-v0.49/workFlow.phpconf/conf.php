...
if ( $opt === "addFlagSave")//增加子项后台
{
    ...
    if (!Empty( $flowChartName ) && !Empty( $fileBody ) && Empty( $srcFileName )  )
    {
        ...
        //if ( preg_match('/<img.+src=\"?(.+\.(jpg|jpeg|gif|bmp|bnp|png))\"?.+>/i',$fileBody,$match) )
        //{
        //    $srcFilePath = $match[1];
    ...
    if ($GLOBAL['G_DB_OBJ']->executeSql($sqlStr) === 1 )
    {
        ...
        echo getAlertFormLocationJSCode('/workFlow.php?opt=editFlag&departmentID='.$departmentID.'&fileID='.$fileID,"填加成功","","parent.parent.leftFrame.location.reload(true);");
    }
    else 
    {
...
if ( $opt === "editFlagSave")//增加子项后台
{
    ...
    if ($GLOBAL['G_DB_OBJ']->executeSql($sqlStr) === 1 )
    {
        $fileID=$GLOBAL['G_DB_OBJ']->getFieldValue("select fileID from fileListOther where name='".$flowChartName."' and srcFilePath='nofile' and srcFileName='".$srcFileName."' and departmentID='".$departmentID."' and fileType='workFlow'");
        echo getAlertFormLocationJSCode('/workFlow.php?opt=readFlag&departmentID='.$departmentID.'&fileID='.$fileID,"","","parent.parent.leftFrame.location.reload(true);");
        exit;
    }
    else
    {
